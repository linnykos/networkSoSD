...
df_param <- cbind(3, 500, 100, 0.1, seq(0, 0.5, length.out = 11), 
                  0.4, 0.1, 0.5)
...
res <- customSimulator::simulator(generator = generator, executor = executor,
                                  ...
                                  cores = ncores, 
                                  filepath = "../results/simulation_rho_switching_tmp.RData",
                                  required_packages = "networkSoSD",
                                  verbose = T)

save.image(paste0("../results/simulation_rho_switching", run_suffix, ".RData"))
