...
plot_func <- function(methods, res_mat, key_mat, df_param, x_var = "rho",
  ...
  for(i in 1:length(idx_vec)){
    graphics::lines(x = as.numeric(df_param[,x_var]), y = res_mat[idx_vec2[i],], col = key_mat$color[idx_vec[i]], 
                    lwd = 2, lty = key_mat$lty[idx_vec[i]])
    graphics::points(x = as.numeric(df_param[,x_var]), y = res_mat[idx_vec2[i],], col = key_mat$color[idx_vec[i]],  
